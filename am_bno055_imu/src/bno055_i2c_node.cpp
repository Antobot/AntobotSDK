...
class BNO055I2CNode {
	...
	void status_callback(const diagnostic_msgs::DiagnosticStatus::ConstPtr& msg);
    private:
        ros::NodeHandle* nh;
	...
	ros::Publisher pub_OpMode;	// Added by Dylan 27-Oct-2021 to read out sensor Operation Mode
	ros::Publisher pub_calib_acc;
	ros::Publisher pub_calib_mag;
	ros::Publisher pub_calib_gyro;
	ros::Publisher pub_acc_rad;
	ros::Publisher pub_mag_rad;
	ros::Publisher pub_sic_mat;

        // Declare subscriber(s)
...
BNO055I2CNode::BNO055I2CNode(int argc, char* argv[]) {
    ...
    pub_OpMode = nh->advertise<std_msgs::UInt8>("mode", 1); // Added by Dylan 27-Oct-2021 to read out sensor operation mode
    pub_calib_acc = nh->advertise<geometry_msgs::Vector3>("acc_offset", 1);
    pub_calib_mag = nh->advertise<geometry_msgs::Vector3>("mag_offset", 1);
    pub_calib_gyro = nh->advertise<geometry_msgs::Vector3>("gyro_offset", 1);
    pub_acc_rad = nh->advertise<std_msgs::UInt16>("acc_rad", 1);
    pub_mag_rad = nh->advertise<std_msgs::UInt16>("mag_rad", 1);
    pub_sic_mat = nh->advertise<std_msgs::UInt16MultiArray>("sic_matrix", 1);


    // Defines subscriber
    sub_status = nh->subscribe("/imu/status", 100, &BNO055I2CNode::status_callback, this);

    srv_reset = nh->advertiseService("reset", &BNO055I2CNode::onSrvReset, this);

    seq = 0;

    // Set to false if suitable calibration parameters have already been saved
    save_calib = false;
...
bool BNO055I2CNode::readAndPublish() {
    ...
    imu_bno055::IMUCalibParam calib_params;

    try {
        record = imu->read();
	calib_params = imu->read_calib_data();
    ...
    double angular_velocity_cov = 5.9644*1e-8; //angular velocity covariance and orientation covariance are the same in this case
    msg_data.orientation_covariance[0] = angular_velocity_cov;
    msg_data.orientation_covariance[4] = angular_velocity_cov;
    msg_data.orientation_covariance[8] = angular_velocity_cov;
    
    msg_data.angular_velocity_covariance[0] = angular_velocity_cov;
    msg_data.angular_velocity_covariance[4] = angular_velocity_cov;
    msg_data.angular_velocity_covariance[8] = angular_velocity_cov;
    
    double linear_acceleration_cov = 2.89*1e-8; //computed from the data sheet
    msg_data.linear_acceleration_covariance[0] = linear_acceleration_cov; 
    msg_data.linear_acceleration_covariance[4] = linear_acceleration_cov; 
    msg_data.linear_acceleration_covariance[8] = linear_acceleration_cov; 
    ...
    std_msgs::UInt8 msg_mode; // Added by Dylan 27-Oct-2021 to read out sensor operation mode
    msg_mode.data = record.operation_mode; // Added by Dylan 27-Oct-2021 to read out sensor operation mode

    // Offset parameters (set by calibration)
    geometry_msgs::Vector3 accel_offset;
    accel_offset.x = calib_params.accel_offset_x;
    accel_offset.y = calib_params.accel_offset_y;
    accel_offset.z = calib_params.accel_offset_z;

    geometry_msgs::Vector3 mag_offset;
    mag_offset.x = calib_params.mag_offset_x;
    mag_offset.y = calib_params.mag_offset_y;
    mag_offset.z = calib_params.mag_offset_z;

    geometry_msgs::Vector3 gyro_offset;
    gyro_offset.x = calib_params.gyro_offset_x;
    gyro_offset.y = calib_params.gyro_offset_y;
    gyro_offset.z = calib_params.gyro_offset_z;

    // Radius parameters (set by calibration)
    std_msgs::UInt16 acc_rad;
    acc_rad.data = calib_params.accel_radius;

    std_msgs::UInt16 mag_rad;
    mag_rad.data = calib_params.mag_radius;

    // Soft Iron Calibration (SIC) Matrix values (for calibration)
    std_msgs::UInt16MultiArray sic_mat_msg;
    std::vector<uint16_t> sic_mat(9);
    sic_mat[0] = calib_params.sic_0; sic_mat[1] = calib_params.sic_1; sic_mat[2] = calib_params.sic_2;
    sic_mat[3] = calib_params.sic_3; sic_mat[4] = calib_params.sic_4; sic_mat[5] = calib_params.sic_5;
    sic_mat[6] = calib_params.sic_6; sic_mat[7] = calib_params.sic_7; sic_mat[8] = calib_params.sic_8;
    sic_mat_msg.data = sic_mat;
    ...
    pub_OpMode.publish(msg_mode); // Added by Dylan 27-Oct-2021 to read out sensor operation mode
    pub_calib_acc.publish(accel_offset);
    pub_calib_mag.publish(mag_offset);
    pub_calib_gyro.publish(gyro_offset);
    pub_acc_rad.publish(acc_rad);
    pub_mag_rad.publish(mag_rad);
    pub_sic_mat.publish(sic_mat_msg);
...
void BNO055I2CNode::stop() {
    ...
    if(pub_OpMode) pub_OpMode.shutdown(); // Added by Dylan 27-Oct-2021 to stop operation mode publisher
}

void BNO055I2CNode::status_callback(const diagnostic_msgs::DiagnosticStatus::ConstPtr& msg) {
    imu_bno055::IMUCalibParam calib_params;
    int calib_status;
    calib_status = std::stoi(msg->values[0].value);

    ROS_INFO("Calib status: %d", calib_status);

    if (calib_status == 255 && save_calib){
	ROS_INFO("Perfect calibration!");

        // Read data from registers        
        calib_params = imu->read_calib_data();

        // Save data to file
	imu->write_calib_params_to_file(calib_params);

        // Don't save calib any more
        save_calib = false;
    }
}
